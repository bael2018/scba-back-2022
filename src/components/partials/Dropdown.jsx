import {
    ...
    setStatus,
    setSubCategory,
} from "../../store/slices/app_slices/appSlice";
...
const Dropdown = ({ data, init, state }) => {
    ...
    const changeTextHandler = (value) => {
        ...
        } else if (init === rootConstants.subCategory) {
            setView(!view);
        ...
        <div className={cls.dropdown}>
            ...
            <div
                className={
                    data?.length > 3
                        ? view
                            ? `${cls.dropdown__body} ${cls.dropdown__body_scroll}`
                            : cls.dropdown__body
                        : view
                        ? `${cls.dropdown__body} ${cls.dropdown__body_active}`
                        : cls.dropdown__body
                }
            >
                {data?.length ? (
                    <ul>
                        {data.map(({ subCategory, id }) => {
                            return (
                                <li
                                    className={` ${
                                        subCategory === state && cls.activeItem
                                    }`}
                                    key={id}
                                    onClick={() =>
                                        changeTextHandler(subCategory)
                                    }
                                >
                                    {subCategory}
                                </li>
